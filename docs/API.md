...
>
> Generated: 2026-08-27T05:54:56.238Z
> Total endpoints: 353

## Which API doc do I read?

- **This file (`API.md`)**: the public REST API served at `https://cryptocurrency.cv`. Every endpoint, parameter, and response we expose.
- [`API-INVENTORY.md`](API-INVENTORY.md): gap analysis of the third-party providers we consume (endpoints used, available-but-unused, missing features).
- [`EXTERNAL-API-ROUTES.md`](EXTERNAL-API-ROUTES.md): catalog of every external API and RSS feed the platform calls.
- [`apis/`](apis/README.md): that external catalog split into one file per category.
- [`SOURCES.md`](SOURCES.md): the news feeds and market-data providers, with keyless / free-tier status.

Note for maintainers: this file is regenerated by `node scripts/generate-api-docs.js`; keep this block in the generator template if it is ever dropped.
